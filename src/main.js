...
console.log(`Province: ${provinceCodes.join(', ')} | ATECO: ${ateco||'tutti'} | Forma: ${formaGiuridica||'tutte'} | Stato: ${statoImpresa}`);
    ...
    async requestHandler({ page, request, log, addRequests }) {
        ...
        for (const sel of cookieSelectors) {
            try {
                ...
                if (await btn.isVisible({ timeout: 3000 })) {
                    await btn.click();
                    log.info(`Cookie banner dismissed: ${sel}`);
        ...
        const { items, totalPages } = await page.evaluate(() => {
            ...
            const items = cards.map(card => {
                ...
                ) || [...card.querySelectorAll('td')][0]?.textContent.trim() || '';
        ...
        if (pageNum === 1 && totalPages > 1) {
            const nextPages = [];
            for (let p = 2; p <= totalPages && (collected + nextPages.length * 20) < maxItems; p++) {
